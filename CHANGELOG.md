...
### Changed
- **jimmy-core-preferences** v3.11.0 -> v3.12.0 — Added **R-45** (CODEX Quota Fallback — Reroute to Anthropic-Family Agent via AOP Real, Protect Magneto Context Window). **Origin:** 2026-04-10, Jimmy's thread entry `2026-04-10-jimmy-01` in `bdvx-pjt-threads-jimmy-magneto.md` at 14:30, verbatim: *"Se acontecer de você receber o erro ou perceber que deu limite de uso excedido, então você deve rotear para AOP REAL, porém para o Agente adequado da Anthropic, SEM PREJUDICAR sua janela de contexto. Sua janela de contexto é extremamente importante e precisa ser mantida o máximo possível."* Jimmy explicitly approved Section R promotion in same thread verbatim: *"R-45 promotion formal Section R (CODEX quota fallback) [...] aprovo. Importante não termos o processo stopado por conta que o CODEX excedeu limite de uso."* Promotion deferred from 14:30 pause to post-resume execution at 16:10 in work session `3d3513e8-4177-43ea-9874-8c4adc3ff6ac`. **Rule:** when a CODEX dispatch returns a rate-limit/quota-exceeded error, or Magneto detects usage is at the limit, the blocked task MUST reroute via AOP real to an Anthropic-family reviewer agent, NOT execute inline in Magneto's context (inline execution is forbidden because it defeats context-window preservation — the core constraint of this rule). Includes 3 detection triggers, 6-step mandatory fallback action (no inline, reroute via AOP real, reuse same reviewer session doc per G-05, same token economy per G-09, brief thread note, escalate to Jimmy only if both CODEX and Anthropic-family are unavailable), critical context-window protection clauses (no reading full targets in Magneto, no inline reviewer persona, no simulated review in Magneto thought process), post-fallback behavior (temporary rerouting, CODEX resumes by default when available, G-04/G-05/G-09 all preserved), why section (process continuity + context preservation are both protected), how-to-apply guidance, and relationship to existing rules (does NOT replace G-04/G-05/G-09; reinforces R-44 continuous-sync because fallback dispatch is a meaningful state change). Mirrored as non-negotiable section "CODEX Quota Fallback (MANDATORY — R-45)" in `C:/ai/CLAUDE.md`, `C:/ai/AGENTS.md`, and `C:/ai/GEMINI.md`.
...
### Added
- **bi-designerx** v0.1.0 — NEW SKILL. End-to-end BI dashboard design workflow for non-designers. AI-driven design generation in Paper.design with CEM system. Authorized by Jimmy.
...
## [2.23.1] - 2026-03-23
### Changed
- **daily-doc-information** v1.4.0 -> v1.5.0 — FND-0024: Added `llm_model` field to session doc template, universal inputs (I-07), placeholder system, I/O contract, and test fixtures. Captures specific LLM model ID (e.g., `claude-opus-4-6`) at session creation for traceability.
